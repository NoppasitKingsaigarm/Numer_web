...
let fx2 = ""
let Intes = ""
class CompoSimson extends Component {
    constructor(props) {
        super(props);
        this.state = {
            users:"",
    ...
    {   
        ...
        if( this.state.check == 1)
        {
            for (var i=1 ; i<n ; i++) {
                ...
                if(counter%2==1){
                    sum += 4 * funcsum2(counter);
                  }
                  else{
                    sum += 2 * funcsum2(counter);
                  }
                  counter += h
            }
            I = (h / 3) * (funcsum2(a) + funcsum2(b) + sum)
            ...
            if(counter%2==1){
                sum += 4 * funcsum(counter);
              }
              else{
                sum += 2 * funcsum(counter);
              }
              counter += h
        }
        I = (h / 3) * (funcsum(a) + funcsum(b) + sum)
    ...
    componentDidMount(){
        axios.get("http://192.168.99.100:3001/api/docomposimson").then(res=>{
            ...
            <div className="textbox1" style={{paddingLeft:'40px'}}>
                ...
                <center>
                    <label style= {{paddingleft:'2px'}}>Input : </label>
                        <Input placeholder="Simson's rlue" onChange={this.Myfx}  style={{width:'500px ',marginRight:'20px'}}/><br/><br/>
