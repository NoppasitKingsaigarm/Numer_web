...
class falsePo extends Component
{
    ...
    falseposition = () => {
        ...
        const funtionxm = (xl, xr) => ((xl * funtion(this.state.fx,parseFloat(xr))) - (xr * funtion(this.state.fx, parseFloat(xl))) ) / (funtion(this.state.fx, parseFloat(xr)) - funtion(this.state.fx, parseFloat(xl)))
        const funtionxmm = (xl, xr) => ((xl * funtion(fx2,parseFloat(xr))) - (xr * funtion(fx2, parseFloat(xl))) ) / (funtion(fx2, parseFloat(xr)) - funtion(fx2, parseFloat(xl)))
        var xm_new
        var xm_old
        var i = 0
        var ans
        console.log(funtion(this.state.fx,parseFloat(xr)))
        console.log(funtionxm(parseFloat(xl),parseFloat(xr)))
        if(this.state.check == 1)
        {
            while (i <= 1|| error(xm_new, xm_old) > 0.000001){
                xm_old = xm_new
                xm_new = funtionxmm(parseFloat(xl), parseFloat(xr))
    ...
    componentDidMount(){
        axios.get("http://192.168.99.100:3001/api/dofalseposition").then(res=>{
        ...
        <div className="textbox1" style={{paddingLeft:'40px'}}>
            ...
            <center>
                <label style= {{paddingleft:'2px'}}>Input : </label>
                    <Input placeholder="False-Position" onChange={this.Myfx} style={{width:'500px ',marginRight:'20px'}}/><br/><br/>
                ...
                <label>XR : </label>
                    ...
                    <Button type="primary" onClick={this.falseposition} style={{backgroundColor:'#FFA500',color:'#000000',borderColor:'#FFD700'}} >Summit</Button><br/><br/>
                ...
                <div className= "mongoDB">
                        ...
                        <button onClick={this.data2}>fx: {this.state.fxx} <br/> xl: {this.state.xll} xr: {this.state.xrr} </button>
                </div> 
