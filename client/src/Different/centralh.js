...
class  Centralh extends Component {
    ...
    centrall = () =>
    {
        ...
        {
            if(d==1){
                y = (funtion(fx2,x+(1*h)) - funtion(fx2,x-(1*h))) / (2*h)
              }
              else if(d==2){
           ...
           if(d==1){
            y = (funtion(fx,x+(1*h)) - funtion(fx,x-(1*h))) / (2*h)
          }
          else if(d==2){
    ...
    componentDidMount(){
        axios.get("http://192.168.99.100:3001/api/docentralh").then(res=>{
            ...
            <div className="textbox1" style={{paddingLeft:'40px'}}>
                ...
                <center>
                    <label style= {{paddingleft:'2px'}}>Input : </label>
                        <Input placeholder="Centralh " onChange={this.Myfx}  style={{width:'500px ',marginRight:'20px'}}/><br/><br/>
