...
class Forwardh extends Component {
    ...
    forwardd = () =>
    {
        ...
        {
            if(d==1){
                y = (funtion(fx2,(x+h)) - funtion(fx2,x)) / h
              }
              else if(d==2){
           ...
           if(d==1){
            y = (funtion(fx,(x+h)) - funtion(fx,x)) / h
          }
          else if(d==2){
            y = ((funtion(fx,(x+(2*h))) - 2*funtion(fx,(x+(h))) + funtion(fx,x)) / Math.pow(h,2))
          }
          else if(d==3){
            y=((funtion(fx,(x+(3*h))) - 3*funtion(fx,(x+(2*h))) + 3*funtion(fx,(x+(h))) - funtion(fx,x)) / Math.pow(h,3))
          }
          else if(d==4){
            y=((funtion(fx,(x+(4*h))) - 4*funtion(fx,(x+(3*h))) + 6*funtion(fx,(x+(2*h)))- 4*funtion(fx,(x+(h))) + funtion(fx,x)) / Math.pow(h,4))
          }
    ...
    componentDidMount(){
        axios.get("http://192.168.99.100:3001/api/doforwardh").then(res=>{
            ...
            <div className="textbox1" style={{paddingLeft:'40px'}}>
                ...
                <center>
                    <label style= {{paddingleft:'2px'}}>Input : </label>
                        <Input placeholder="first forward" onChange={this.Myfx}  style={{width:'500px ',marginRight:'20px'}}/><br/><br/>
