...
let fx = ""
let fx2 = ""
class  Backwardhh extends Component {
    constructor(props) {
        super(props);
        this.state = {
            users:"",
    ...
    backwardd = () =>
    {
        ...
        {
            if(d==1){
                y = (3*funtion(fx2,x) - 4*funtion(fx2,x-(1*h)) + funtion(fx2,x-(2*h))) / (2*h)
              }
              else if(d==2){
                y = (2*funtion(fx2,x) - 5*funtion(fx2,x-(1*h)) + 4*funtion(fx2,x-(2*h)) - funtion(fx2,x-(3*h))) / Math.pow(h, 2)
              }
              else if(d==3){
                y= (5*funtion(fx2,x) - 18*funtion(fx2,x-(1*h)) + 24*funtion(fx2,x-(2*h)) - 14*funtion(fx2,x-(3*h)) + 3*funtion(fx2,x-(3*h))) / (2*Math.pow(h, 3))
              }
              else if(d==4){
                y= (3*funtion(fx2,x) - 14*funtion(fx2,x-(1*h)) + 26*funtion(fx2,x-(2*h)) - 24*funtion(fx2,x-(3*h)) + 11*funtion(fx2,x-(4*h)) - 2*funtion(fx2,x-(5*h))) / Math.pow(h, 4) 
              }
              for(var i=1 ;i<d;i++)
              {
           ...
           if(d==1){
            y = (3*funtion(fx,x) - 4*funtion(fx,x-(1*h)) + funtion(fx,x-(2*h))) / (2*h)
          }
          else if(d==2){
            y = ((2*funtion(fx,x)) - (5*funtion(fx,x-(1*h))) + (4*funtion(fx,x-(2*h))) - (funtion(fx,x-(3*h)))) / Math.pow(h, 2)
          }
          else if(d==3){
            y= (5*funtion(fx,x) - 18*funtion(fx,x-(1*h)) + 24*funtion(fx,x-(2*h)) - 14*funtion(fx,x-(3*h)) + 3*funtion(fx,x-(3*h))) / (2*Math.pow(h, 3))
          }
          else if(d==4){
            y= (3*funtion(fx,x) - 14*funtion(fx,x-(1*h)) + 26*funtion(fx,x-(2*h)) - 24*funtion(fx,x-(3*h)) + 11*funtion(fx,x-(4*h)) - 2*funtion(fx,x-(5*h))) / Math.pow(h, 4)  
          }
            ...
            <div className="textbox1" style={{paddingLeft:'40px'}}>
                ...
                <center>
                    <label style= {{paddingleft:'2px'}}>Input : </label>
                        <Input placeholder="back forward 2" onChange={this.Myfx}  style={{width:'500px ',marginRight:'20px'}}/><br/><br/>
